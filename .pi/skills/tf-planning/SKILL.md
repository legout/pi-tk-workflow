...
### Procedure: Follow-ups Scan

**Purpose**: Scan implemented tickets and automatically invoke `/tf-followups` to create follow-up tickets from review Warnings/Suggestions.

**Input**: Optional flags

**Flags**:
- `--dry-run` - Show what would be done without creating tickets (default behavior)
- `--apply` - Actually create follow-up tickets (requires explicit opt-in)
- `--since <date>` - Only scan tickets implemented since date (e.g., `2026-01-15`)
- `--ticket <id>` - Scan a specific ticket instead of all recent
- `--limit <n>` - Maximum number of tickets to scan (default: 10)
- `--json` - Output structured JSON instead of human-readable table (CI/automation friendly)
- `--stop-on-error` - Abort scan if any `tk create` fails (default: continue with error logging)
...
2. **Discover implemented tickets**:
   - Scan `{ticketsDir}/*/` for ticket artifact directories
   - **"Implemented ticket" heuristic (MVP)**: A ticket is considered implemented/closed if it has:
     - `close-summary.md` - Ticket was formally closed
   - Future: May support `--implemented-heuristic` flag for alternative detection methods (e.g., `implementation+review`)

3. **Filter tickets** (if flags provided):
   - `--since`: Check `close-summary.md` timestamp
   - `--ticket`: Use only the specified ticket ID
   - `--limit`: Stop after N tickets, default 10 (ordered by close date, newest first)

4. **Idempotency check and review parsing**:
   - Skip tickets that already have `followups.md` (already processed)
   - For remaining tickets, check for `review.md`
   - If no `review.md`: will record skip reason in `followups.md`
   - If `review.md` exists: parse Warnings and Suggestions sections

5. **Dry-run output (default or `--dry-run`)**:
   - Print table: Ticket ID | Warnings | Suggestions | Would Create
   - Show total count of follow-up tickets that would be created
   - Exit without making changes
   - Emit: "Re-run with `--apply` to create tickets"

6. **Apply mode (`--apply`)**:
   - For each implemented ticket missing `followups.md`:
     - If no `review.md` exists: write `followups.md` with "Skipped: no review.md present."
     - If `review.md` exists but no Warnings/Suggestions: write `followups.md` with "No warnings or suggestions found in review."
     - Otherwise: invoke **Procedure: Follow-up Creation** with the ticket's `review.md` path
   - **Write `followups.md` atomically** (temp file + rename) for each ticket:
     ```markdown
     ...
     - Reason: {explanation}
     ```
   - Handle errors gracefully: log and continue (unless `--stop-on-error`)
   - Print summary (table or JSON if `--json`)
   - Write scan log to `{ticketsDir}/followups-scan-{timestamp}.md` for historical tracking

**Safety Defaults**:
- **Dry-run by default**: Always preview before creating tickets
- **Explicit `--apply` required**: Must opt-in to create tickets and write `followups.md`
- **Implemented ticket detection**: Only process tickets with `close-summary.md` (conservative, avoids false positives)
- **Idempotent by artifact**: Tickets with existing `followups.md` are skipped automatically
- **Atomic writes**: Each `followups.md` is written atomically (temp + rename) to prevent partial files on crash
- **Graceful degradation**: Continue on errors (unless `--stop-on-error`), log skips with reasons

**Example Usage**:

```bash
# Preview what follow-ups would be created
/tf-followups-scan

# Preview for last 30 days
/tf-followups-scan --since 2026-01-01

# Actually create the follow-up tickets
/tf-followups-scan --apply

# Scan specific ticket
/tf-followups-scan --ticket pt-abc123 --apply
```

**Relationship to Follow-up Creation**:
- **Single-ticket mode**: `/tf-followups <ticket-or-path>` - Creates follow-ups for one specific review
- **Scan mode**: `/tf-followups-scan` - Bulk operation across multiple implemented tickets
- Scan mode delegates to the single-ticket Follow-up Creation procedure for each ticket
