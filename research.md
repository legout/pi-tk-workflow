...
- Required Pi extensions referenced: `pi-subagents` and `pi-model-switch`; optional: `pi-review-loop` (used by the `--final-review-loop` flag) and `pi-mcp-adapter` for MCP tooling. `bin/irf setup` installs these and can configure MCP servers (context7/exa/grep_app + optional ZAI endpoints).
